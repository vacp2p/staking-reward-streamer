...
methods {
    ...
    function accountSlashedAmount(address) external returns (uint256) envfree;
...
function getSlashAmountForAccount(address account) returns uint256 {
    uint256 slashAmount = karma.accountSlashedAmount(account);
    return slashAmount;
}
