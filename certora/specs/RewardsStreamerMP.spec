...
function getVaultMP(address vault) returns uint256 {
    uint256 mp;
    _, _, mp, _, _, _ = streamer.vaultData(vault);
    return mp;
}
...
invariant vaultMPLessEqualVaultMaxMP(address vault)
  to_mathint(getVaultMP(vault)) <= to_mathint(getVaultMaxMP(vault))
  filtered {
    f -> f.selector != sig:upgradeToAndCall(address,bytes).selector
  }

invariant vaultMPGreaterEqualVaultStakedBalance(address vault)
  to_mathint(getVaultMP(vault)) >= to_mathint(getVaultStakedBalance(vault))
  filtered {
    f -> f.selector != sig:upgradeToAndCall(address,bytes).selector
  }
...
rule stakingMintsMultiplierPoints1To1Ratio {
  ...
  multiplierPointsBefore = getVaultMP(e.msg.sender);
  ...
  multiplierPointsAfter = getVaultMP(e.msg.sender);
...
rule stakingGreaterLockupTimeMeansGreaterMPs {
  ...
  multiplierPointsAfter1 = getVaultMP(e.msg.sender);

  stake(e, amount, lockupTime2) at initalStorage;
  multiplierPointsAfter2 = getVaultMP(e.msg.sender);
